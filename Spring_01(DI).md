## Spring_01(DI, Dependency Injection)

> 의존성(Dependency)은 하나의 객체가 다른 객체 없이 제대로 된 역할을 할 수 없다는 것을 의미한다. 즉, 하나의 객체가 다른 객체의 상태에 따라 영향을 받는 것을 의미한다. 
>
> 주입(Injection)은 말 그대로 외부에서 '밀어넣는 것'을 의미한다. 
>
> 의존성과 주입를 결합해서 생각해 보면 **어떤 객체가 필요한 객체를 외부에서 밀어 넣는다**는 의미이다.
